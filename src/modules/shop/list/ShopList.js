...
export const ShopList = () => {
    ...
    const deleteData = async () => {
        setIsLoading(true)
        const result = await shopService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await shopService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await shopService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await shopService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await shopService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await shopService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        const result = await shopService.index(dispatch, paginateParams);
        if (result.status === 200) {
            setTotal(
                result.data.total
            );
        ...
        setIsLoading(false)
        if(getData(shopPayload.columnsName) == null){
            setData(shopPayload.columnsName, shopPayload.columns)
        }
    ...
    useEffect(()=>{
        setData(shopPayload.columnsName, columns)
    },[columns])
                                                ...
                                                {columns.map((column) => {
                                                    ...
                                                    const switchCase = ({ column, value }) => {
                                                        switch (column.id) {
                                                            ...
                                                            case "option":
                                                                return (
                                                                    <NavigateId url={`${paths.shop}/${row.id}`} id={row.id} />
                                                                )
                                                            default:
                                                                return value;
                                                        }
