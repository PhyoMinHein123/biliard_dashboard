...
export const UserList = () => {
    ...
    const deleteData = async () => {
        setIsLoading(true)
        const result = await userService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await userService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await userService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await userService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await userService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await userService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        ...
        setIsLoading(false)
        if(getData(userPayload.columnsName) == null){
            setData(userPayload.columnsName, userPayload.columns)
        }
    ...
    useEffect(()=>{
        setData(userPayload.columnsName, columns)
    },[columns])
