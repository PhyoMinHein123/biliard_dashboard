...
import { setPaginate } from "../tableSlice";
...
export const TableList = () => {
    ...
    const reloadData = () =>{
        ...
        dispatch(setPaginate(tablePayload.paginateParams));
    }

    const deleteData = async () => {
        setIsLoading(true)
        const result = await tableService.destory(dispatch, selectedId)
    ...
    const exportExcelData = async () => {
        await tableService.exportexcel(dispatch)
    }

    const exportExcelParamsData = async () => {
        await tableService.exportexcelparams(dispatch, paginateParams)
    }

    const exportPdfData = async () => {
        await tableService.exportpdf(dispatch)
    }

    const exportPdfParamsData = async () => {
        await tableService.exportpdfparams(dispatch, paginateParams)
    }
    ...
    const importData = async (e) => {
        ...
        const create = await tableService.import(formData, dispatch);
    ...
    const loadingData = useCallback(async () => {
        const result = await tableService.index(dispatch, paginateParams);
        if (result.status === 200) {
            setTotal(
                result.data.total
            );
    ...
    useEffect(()=>{
        setData(tablePayload.columnsName, columns)
    },[columns])
                                ...
                                <TableRow>
                                    <TableCell colSpan={12}>
                                        <Grid container spacing={2} direction="row" sx={{ paddingTop: 1 }}>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={7} xl={7} direction="row" justifyContent="flex-start" alignTables="center">
                                                
                                                <Grid table xs={1}>
                                                    <TableCustomizeSetting payload={tablePayload.columns} columns={columns} setColumns={(e)=>setColumns(e)} />
                                                </Grid>
                                            ...
                                            <Grid container spacing={0.5} xs={12} sm={12} md={12} lg={5} xl={5} direction="row" justifyContent="flex-end" alignTables="center">
                                                ...
                                                {columns.map((column) => {
                                                    ...
                                                    const switchCase = ({ column, value }) => {
                                                        switch (column.id) {
                                                            ...
                                                            case "option":
                                                                return (
                                                                    <NavigateId url={`${paths.table}/${row.id}`} id={row.id} />
                                                                )
                                                            default:
                                                                return value;
                                                        }
